...
struct CellInfo {
    static const int n = Descriptor<T>::q + Descriptor<T>::ExternalField::numScalars;
};
...
public:
    ...
    T& operator[](plint iPop) {
        PLB_PRECONDITION( iPop < Descriptor<T>::q );
        return f[iPop];
    }
    ...
    Array<T,Descriptor<T>::q>& getRawPopulations() {
        return f;
    }
    /// Another way to get direct, const access to the f's, as in operator[]
    Array<T,Descriptor<T>::q> const& getRawPopulations() const {
        return f;
    }
...
public:
    ...
    void collide(BlockStatistics<T>& statistics) {
    ...
    void computeDeviatoricStress (
            Array<T,SymmetricTensor<T,Descriptor>::n>& PiNeq ) const
    {
        PLB_PRECONDITION( dynamics );
        dynamics->computeDeviatoricStress(*this, PiNeq);
    }
     ...
     * object.
     */
    void getPopulations(Array<T,Descriptor<T>::q>& f) const {
        PLB_PRECONDITION( dynamics );
     ...
     * */
    void defineDeviatoricStress (
            Array<T,SymmetricTensor<T,Descriptor>::n> const& PiNeq )
    {
        PLB_PRECONDITION( dynamics );
        dynamics->defineDeviatoricStress(*this, PiNeq);
    }
     ...
     * object.
     */
    void setPopulations(Array<T,Descriptor<T>::q> const& f) {
        PLB_PRECONDITION( dynamics );
...
private:
    ...
    void iniExternal();
private:
    Array<T,Descriptor<T>::q> f;         ///< distribution functions
