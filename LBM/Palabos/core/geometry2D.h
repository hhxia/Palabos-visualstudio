...
struct Box2D {
    ...
    plint getMaxWidth() const { return max(getNx(), getNy()); }
...
struct DotList2D {
    ...
    /// Return same poplint list, shifted by (deltaX,deltaY)
...
/// Decide if lattice poplint is contained in 2D box, boundaries inclusive
