...
MultiProcessing3D<T,OriginalGenerator,MutableGenerator>::MultiProcessing3D (
...
template<typename T, class OriginalGenerator, class MutableGenerator>
void MultiProcessing3D<T,OriginalGenerator,MutableGenerator>::subdivideGenerator()
{
    ...
    for (pluint iMulti=0; iMulti<multiBlocks.size(); ++iMulti) {
        ...
        for (pluint rBlock=0; rBlock < relevantBlocks.size(); ++rBlock) {
            ...
            BlockParameters3D const& params = multiBlocks[iMulti] -> getParameters(iBlock);
            switch (generator.appliesTo()) {
                case BlockDomain::bulk:
                    domainsWithId[iMulti].push_back(DomainAndId3D(params.getBulk(),iBlock));
                    break;
                case BlockDomain::bulkAndEnvelope:
                    domainsWithId[iMulti].push_back(DomainAndId3D(params.getEnvelope(),iBlock));
                    break;
                case BlockDomain::envelope:
                    ...
                    if (iMulti==referenceBlock) {
                        ...
                        for (pluint iEnvelope=0; iEnvelope<envelopeOnly.size(); ++iEnvelope) {
                            domainsWithId[iMulti].push_back(DomainAndId3D(envelopeOnly[iEnvelope], iBlock));
                        }
                    }
    ...
    if ( BlockDomain::usesEnvelope(generator.appliesTo()) ) {
        ...
        PeriodicitySwitch3D<T> const& periodicity = firstMultiBlock->periodicity();
...
template<typename T, class OriginalGenerator, class MutableGenerator>
void MultiProcessing3D<T,OriginalGenerator,MutableGenerator>::extractGeneratorOnBlocks (
...
template<typename T, class OriginalGenerator, class MutableGenerator>
void MultiProcessing3D<T,OriginalGenerator,MutableGenerator>::adjustCoordinates() {
    for (pluint iGenerator=0; iGenerator<retainedGenerators.size(); ++iGenerator) {
        ...
        BlockParameters3D const& params = firstMultiBlock->getParameters(firstNumber);
        Box3D const& envelope = params.getEnvelope();
...
std::vector<MutableGenerator*> const&
    MultiProcessing3D<T,OriginalGenerator,MutableGenerator>::getRetainedGenerators() const
{
...
std::vector<std::vector<plint> > const&
    MultiProcessing3D<T,OriginalGenerator,MutableGenerator>::getAtomicBlockNumbers() const
{
    return atomicBlockNumbers;
}

template<typename T, class OriginalGenerator, class MutableGenerator>
std::vector<MultiBlock3D<T>*> 
    MultiProcessing3D<T,OriginalGenerator,MutableGenerator>::multiBlocksWhichRequireUpdate() const
{
    std::vector<MultiBlock3D<T>*> multiBlocksModifiedByProcessor;
...
template<typename T, class OriginalGenerator, class MutableGenerator>
void MultiProcessing3D<T,OriginalGenerator,MutableGenerator>::updateEnvelopesWhereRequired()
{
    std::vector<MultiBlock3D<T>*> multiBlocks = multiBlocksWhichRequireUpdate();
...
{
    MultiProcessing3D<T, DataProcessorGenerator3D<T> const, DataProcessorGenerator3D<T> >
        multiProcessing(generator, multiBlocks);
    std::vector<DataProcessorGenerator3D<T>*> const& retainedGenerators = multiProcessing.getRetainedGenerators();
    std::vector<std::vector<plint> > const& atomicBlockNumbers = multiProcessing.getAtomicBlockNumbers();

    for (pluint iGenerator=0; iGenerator<retainedGenerators.size(); ++iGenerator) {
        std::vector<AtomicBlock3D<T>*> extractedAtomicBlocks(multiBlocks.size());
...
void executeDataProcessor( ReductiveDataProcessorGenerator3D<T>& generator,
                           std::vector<MultiBlock3D<T>*> multiBlocks )
{
    MultiProcessing3D<T, ReductiveDataProcessorGenerator3D<T>, ReductiveDataProcessorGenerator3D<T> >
        multiProcessing(generator, multiBlocks);
    std::vector<ReductiveDataProcessorGenerator3D<T>*> const& retainedGenerators = multiProcessing.getRetainedGenerators();
    std::vector<std::vector<plint> > const& atomicBlockNumbers = multiProcessing.getAtomicBlockNumbers();
...
void addInternalProcessor( DataProcessorGenerator3D<T> const& generator,
                           std::vector<MultiBlock3D<T>*> multiBlocks, plint level )
{
    MultiProcessing3D<T, DataProcessorGenerator3D<T> const, DataProcessorGenerator3D<T> >
        multiProcessing(generator, multiBlocks);
    std::vector<DataProcessorGenerator3D<T>*> const& retainedGenerators = multiProcessing.getRetainedGenerators();
