...
void ZouHeClosure ( Cell<T,Descriptor>& cell, Dynamics<T,Descriptor> const& dynamics )
{
    ...
    // (directions 3,4,5)
    std::vector<int> missingIndexes = indexTemplates::subIndexOutgoing<L,direction,orientation>();
    ...
    for (pluint iPop = 0; iPop < missingIndexes.size(); ++iPop)
    {
        ...
        cell[pop] = cell[opp]
            - dynamics.computeEquilibrium(opp, L::rhoBar(rho), j, jSqr)
            + dynamics.computeEquilibrium(pop, L::rhoBar(rho), j, jSqr);
    }
...
template<typename T, template<typename U> class Descriptor, int direction, int orientation>
ZouHeVelocityDynamics<T,Descriptor,direction,orientation>::ZouHeVelocityDynamics(Dynamics<T,Descriptor>* baseDynamics)
...
template<typename T, template<typename U> class Descriptor, int direction, int orientation>
ZouHePressureDynamics<T,Descriptor,direction,orientation>::ZouHePressureDynamics(Dynamics<T,Descriptor>* baseDynamics)
