...
/// Default implementation of ExternalForceAccess3D: return 0
template<typename T, template<typename U> class Descriptor, plint numForceComponents>
struct ExternalForceAccess3D {
    static T getComponent(Cell<T,Descriptor> const& cell, plint iD) {
        return T();
    }
};

/// Specialization of ExternalForceAccess3D: return force from external scalar if available
template<typename T, template<typename U> class Descriptor>
struct ExternalForceAccess3D<T,Descriptor,3> {
    static T getComponent(Cell<T,Descriptor> const& cell, plint iD) {
...
/// Automatic instantiation of ExternalForceAccess3D, depending on the Descriptor
template<typename T, template<typename U> class Descriptor>
T getExternalForceComponent3D(Cell<T,Descriptor> const& cell, plint iD) {
    return ExternalForceAccess3D<T, Descriptor, Descriptor<T>::ExternalField::sizeOfForce>::getComponent(cell,iD);
}
        ...
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                ...
                for (plint iPop = 0; iPop < D::q; ++iPop) {
                    ...
                    for (plint iSpecies=0; iSpecies<numSpecies; ++iSpecies) {
                        Cell<T,Descriptor> const& cell = lattices[iSpecies]->get(nextX,nextY,nextZ);
                ...
                for (plint iSpecies=0; iSpecies<numSpecies; ++iSpecies) {
                    ...
                    for (int iD = 0; iD < D::d; ++iD) {
                        ...
                        T forceContribution = getExternalForceComponent3D(cell, iD);
...
template<typename T, template<typename U> class Descriptor>
void ShanChenMultiComponentProcessor3D<T,Descriptor>::getModificationPattern(std::vector<bool>& isWritten) const
{
